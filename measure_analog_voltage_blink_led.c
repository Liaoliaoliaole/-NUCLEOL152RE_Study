/*
Connect multiturn 10 kΩ trimmer to Arduino connector A0 input and use voltage values 0V-3,3V. 
Measure A0 input voltage with Fluke 87 or Fluke 179. 
Switch on led LD2 in Nucleo board when voltage is higher than 1V in input A0. .
*/
...
void delay_Ms(int delay);
  void read_and_print_ADC(uint8_t channel);
...
int main(void)
{
  ...
  while (1)
  {
	  read_and_print_ADC(0);
	  delay_Ms(1000);
	  read_and_print_ADC(0);
	  delay_Ms(1000);

	  float voltage = read_ADC(0); // Read voltage on A0
	  if (voltage > 1.0) {
	   GPIOA->ODR |= GPIO_ODR_ODR_5; // Turn on LD2 LED
	   } else {
	  GPIOA->ODR &= ~GPIO_ODR_ODR_5; // Turn off LD2 LED
	  }
...
void read_and_print_ADC(uint8_t channel)
{
  ...
  sprintf(buf, "A%d=%d", channel, result);
